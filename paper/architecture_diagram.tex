...
\tikzset{
  ...
  pathbox/.style={draw=#1, rounded corners=5pt, line width=1.0pt, dashed, inner sep=10pt},
}
...
% Tactile projection
\node[proj, right=1.4cm of tac_enc, yshift=-0.3cm] (tac_proj) {
  \textbf{Proj}\\[-2pt]
  {\tiny $1024 \to 768$}
};

% RGB projection
\node[proj, right=1.4cm of rgb_enc] (rgb_proj) {
  \textbf{Proj}\\[-2pt]
  {\tiny $1024 \to 768$}
};

% Queries
\node[proj, right=0.8cm of tac_proj, minimum width=1.6cm] (queries) {
  \textbf{Queries}\\[-2pt]
  {\tiny $197 \times 768$}
};

% Memory
\node[proj, right=0.8cm of rgb_proj, minimum width=1.6cm] (memory) {
  \textbf{Memory}\\[-2pt]
  {\tiny $197 \times 768$}
};
...
\node[trunk, right=0.8cm of $(queries.east)!0.5!(memory.east)$,
      minimum height=3.6cm] (trunk) {};

% Trunk title at top
\node[font=\footnotesize\bfseries, anchor=north] at ($(trunk.north)-(0, 0.15)$)
  {Fusion Trunk $\times 4$};

% Bottleneck inside trunk (below title)
\node[bnblock, anchor=north] at ($(trunk.north)-(0, 0.55)$) (bn) {
  \textbf{Bottleneck}\\[-2pt]
  {\tiny $m{=}16, 768$}
};

% Steps below bottleneck
\node[font=\scriptsize, align=left, anchor=north] at ($(bn.south)-(0, 0.15)$) {
  \ding{192} BN $\leftarrow$ Memory\\[-1pt]
  \ding{193} Queries $\leftarrow$ BN\\[-1pt]
  \ding{194} Queries self-attn
};
...
% Pose token
\node[proj, right=1.0cm of trunk, yshift=0.0cm, minimum width=1.4cm] (pose_tok) {
  \textbf{Pose}\\[-2pt]
  \textbf{token}
};

% Pose MLP
\node[head, right=1.2cm of pose_tok, minimum width=1.6cm] (pose_mlp) {
  \textbf{Pose}\\
  \textbf{MLP}
};
...
\draw[arr] (pose_tok) -- (pose_mlp)
  node[midway, below=2pt, lbltiny] {+spatial pool};
...
\draw[darr={black!50}] (bn.north) -- ++(0, 1.0) -| (tap_inject.south)
  node[pos=0.25, right=2pt, lbltiny] {bottleneck};
...
% ============================================================
% MODALITY DROPOUT ANNOTATION (right side of fusion trunk)
% ============================================================

\node[font=\tiny, text=black!45, align=left, anchor=north west,
      inner sep=0pt]
  at ($(trunk.north east)+(0.15, -0.15)$) {
  modality\\dropout:\\[1pt]
  $p_{\mathrm{both}}{=}.55$\\
  $p_{\mathrm{tac}}{=}.35$\\
  $p_{\mathrm{rgb}}{=}.10$
};
